...
class ThirdPersonSysAsUser09(LLMRater):
    def __init__(
        self,
        llm: LLM,
    ...
    def _get_prompt(
        ...
        self.system_prompt = (
            f"You are {name}, a {user.age} years old {gender}."
            f" {name} is {self.adjust_text_in(user.description, do_rename)}\n"
            + (
            ...
            # + f"\n\nYou need to rate the movie taking into account both {name}'s unique"
            # " characteristics and the information provided about the movie."
        )
        ...
        prompt = (
            ...
            " scale of 0 to 9, where 0 is low and 9 is high?"
        )
