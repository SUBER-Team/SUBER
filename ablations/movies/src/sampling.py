...
class SamplingStudy(AbstractCaseStudy):
    ...
    def run(self):
        ...
        NUM_SAMPLED_USERS = 25

        acc = []
        for item_id in tqdm.tqdm(range(env.num_items)):
            users_ids = np.random.choice(
                env.num_users, NUM_SAMPLED_USERS, replace=False
            )
            users_ids_map = {user: i for i, user in enumerate(users_ids)}
            data, vote_average_tmdb = interact_sequential_ids(env, users_ids, [item_id])
