...
class BooksCollectionStudy(AbstractCaseStudy):
    name = "books_collection_random_history_v2"
    ...
    def interact_sequential_ids(
        ...
        for user in users_ids:
            ...
            for item_id in interact_ids:
                ...
                if r == 0.0:
                    r = 4.21  # Amazon est. average
                if item_id in previos_items_ids:
                    r = 5 if positive else 1
    ...
    def run(self):
        ...
        for saga in tqdm.tqdm(sagas_ids):
            ...
            perc_success_yes = (
                ratings_positive >= np.full_like(ratings_positive, 4)
            ).mean()
            perc_success_no = (
                ratings_negative <= np.full_like(ratings_negative, 3)
            ).mean()
            acc_rate_negative.append(perc_success_no)
            acc_rate_positive.append(perc_success_yes)

            data_no_acc.append(data_negative)
            data_yes_acc.append(data_positive)
        ...
        data_no_acc = pd.concat(data_no_acc)
        data_yes_acc = pd.concat(data_yes_acc)
        data_no_acc.to_csv(
