...
class Net(nn.Module):
    def __init__(
        ...
        self.policy_net = nn.Sequential(
            nn.Linear(
                self.user_embedding.embedding_dim + num_items,
                self.latent_dim_pi * 2,
            ),
            nn.ReLU(),
            nn.Linear(self.latent_dim_pi * 2, num_items),
        )
    ...
    def forward(self, features: TensorDict) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        film_seen = features["items_interact"].to(torch.bool).to(torch.float32)

        user_embedding = self.user_embedding(user_id)
        user_embedding_extended = torch.cat([user_embedding, film_seen], dim=1)
        user_bias = self.user_bias(user_id)
        logits = self.policy_net(user_embedding_extended) + user_bias

        if not self.training:
            mask = features["items_interact"].to(dtype=torch.bool)
            logits[mask] = -torch.inf

        return logits, self.value_net(user_embedding_extended)
    ...
    def forward_critic(self, features: TensorDict) -> torch.Tensor:
        ...
        user_embedding = self.user_embedding(user_id)
        user_embedding_extended = torch.cat([user_embedding, film_seen], dim=1)
        return self.value_net(user_embedding_extended)
