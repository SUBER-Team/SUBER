    ...
    def generate_normal(self, seed, num):
        ...
        prompt = guidance(
            (
                ...
                " need to generate a name, an age, an hobby, a job and a detailed,"
                " long and original description that contains the person interests and"
                " secondary hobbies."
                # " Moreover, generate the person's film preferences:"
                # " please specify what genres the person likes and dislikes and explain"
                # " why."
                ...
                + "Job: {{jobs[@index]}}\n"
                + "Genres liked: "
                + "{{#geneach 'this.genres_liked' join=', '  max_iterations=2}}{{select"
                " 'this' options=genres}} {{/geneach}}\n"
                + "Genres disliked: "
                + "{{#geneach 'this.genres_disliked' join=', ' max_iterations=2"
                " }}{{select 'this' options=genres}} {{/geneach }}\n"
...
def arg_parse():
    ...
    parser.add_argument("--split-without-film-interest", type=float, default=1.0)
